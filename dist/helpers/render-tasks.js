export const render = (tasks, tasksContainerElement) => {
    ...
    tasks.forEach((task, index) => {
        ...
        const taskElement = document.createElement("li"); // stwórz element `li`
        if (task.category) {
            taskElement.classList.add(task.category);
        } // jesli `task.category` istnieje to dodaj go do listy
        const id = `task-${index}`; // przypisz jako `id` numer pobrany przy pętli i dodaj go z prefixem `task-`
        const labelElement = document.createElement("label"); // stwórz element `label`
        ...
        const checkboxElement = document.createElement("input"); // stwrórz element `input`
